...
<el-table v-else :data="errors" max-height="520" class="desktop-only"><el-table-column prop="rowNumber" label="行号" width="70"/><el-table-column prop="inpatientNo" label="住院号" width="120"/><el-table-column prop="admissionTimes" label="住院次数" width="90"/><el-table-column prop="patientName" label="患者姓名" width="110"/><el-table-column prop="fieldName" label="字段" width="130"/><el-table-column prop="originalValue" label="原值" min-width="130" show-overflow-tooltip/><el-table-column prop="errorCode" label="错误码" width="150"/><el-table-column prop="errorMessage" label="错误说明" min-width="220"/></el-table>
<div v-if="errors.length" class="mobile-only import-error-mobile-list"><article v-for="item in errors" :key="`${item.rowNumber}-${item.errorCode}-${item.fieldName||''}`" class="import-error-mobile-card"><header><strong>第 {{item.rowNumber}} 条 · {{item.errorCode}}</strong></header><dl><div><dt>住院号</dt><dd>{{item.inpatientNo||'—'}}</dd></div><div><dt>住院次数</dt><dd>{{item.admissionTimes??'—'}}</dd></div><div><dt>患者姓名</dt><dd>{{item.patientName||'—'}}</dd></div><div><dt>字段</dt><dd>{{item.fieldName||'—'}}</dd></div><div><dt>原值</dt><dd>{{item.originalValue||'—'}}</dd></div><div class="wide error"><dt>错误说明</dt><dd>{{item.errorMessage}}</dd></div></dl></article></div>
</el-dialog>
</section>
</template>
