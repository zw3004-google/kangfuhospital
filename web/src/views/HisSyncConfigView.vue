...
const isUnavailable=(row:Config)=>row.syncType==='DISCHARGED_ARREARS'
const rowClassName=({row}:{row:Config})=>isUnavailable(row)?'his-sync-disabled-row':''
...
<el-table v-loading="loading" :data="rows" :row-class-name="rowClassName" stripe class="desktop-only">
<el-table-column label="数据类型" min-width="170"><template #default="s"><strong>{{labels[s.row.syncType as SyncType]}}</strong><el-tag v-if="isUnavailable(s.row)" type="info" size="small" class="unavailable-tag">暂不可用</el-tag></template></el-table-column>
<el-table-column label="自动同步" width="120"><template #default="s"><el-switch v-model="s.row.enabled" :disabled="isUnavailable(s.row)"/></template></el-table-column>
<el-table-column label="每日时间" width="180"><template #default="s"><el-time-picker v-model="s.row.dailyTime" value-format="HH:mm:ss" format="HH:mm" :clearable="false" :disabled="isUnavailable(s.row)"/></template></el-table-column>
...
<el-table-column label="操作" width="100" fixed="right"><template #default="s"><el-button type="primary" link :loading="saving===s.row.syncType" :disabled="isUnavailable(s.row)" @click="save(s.row)">保存</el-button></template></el-table-column>
</el-table>
<div v-loading="loading" class="mobile-only his-sync-mobile-list">
<el-empty v-if="!loading&&!rows.length" description="暂无同步配置"/>
<article v-for="row in rows" :key="row.syncType" class="his-sync-mobile-card" :class="{'his-sync-disabled-card':isUnavailable(row)}">
<header><strong>{{labels[row.syncType]}}</strong><el-tag v-if="isUnavailable(row)" type="info">暂不可用</el-tag><el-tag v-else :type="row.running?'warning':'info'">{{row.running?'运行中':'空闲'}}</el-tag></header>
<div class="his-sync-mobile-controls"><label><span>自动同步</span><el-switch v-model="row.enabled" :disabled="isUnavailable(row)"/></label><label><span>每日同步时间</span><el-time-picker v-model="row.dailyTime" value-format="HH:mm:ss" format="HH:mm" :clearable="false" :disabled="isUnavailable(row)"/></label></div>
...
<footer><el-button type="primary" :loading="saving===row.syncType" :disabled="isUnavailable(row)" @click="save(row)">保存配置</el-button></footer>
</article>
</div>
</section>
</template>
