...
public class DischargeImportService {
    ...
    public Result importFile(MultipartFile file) {
        validateFile(file); List<DischargeImportRow> rows = readRows(file);
    ...
    @Transactional
    public Result importApiRows(List<DischargeImportRow> rows, String transactionCode, String triggerType, Long startedBy) {
        if (rows.isEmpty()) throw new IllegalArgumentException("HIS接口未返回数据");
        try { validateRows(rows); }
        catch (ImportValidationException exception) {
            String failedBatchNo=failedBatches.recordApi("DISCHARGE",transactionCode,triggerType,startedBy,rows.size(),exception.getErrors());
            throw new ImportValidationException(failedBatchNo,exception.getErrors());
        }
        jdbc.sql("SELECT pg_advisory_xact_lock(hashtext('IMPORT_DISCHARGE'))").query((rs,rowNum)->true).single();
        String batchNo = "DIS-API-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"))
        ...
        long batchId = jdbc.sql("""
                ...
                VALUES (:batchNo,'DISCHARGE','API','PROCESSING',:total,:transactionCode,:triggerType,:startedBy) RETURNING id
        ...
        for(DischargeImportRow row:rows){Long id=find(row);boolean exists=id!=null;DoctorMatch doctor=matchDoctor(row.doctorEmployeeNo);if(doctor.matched())matched++;else if(doctor.ambiguous())ambiguous++;else unmatched++;if(id==null)id=insert(row,doctor);else update(id,row,doctor);upsertDischarge(id,row);if(exists)overwritten++;else added++;}
        jdbc.sql("UPDATE import_batch SET status='SUCCESS',success_count=:success,added_count=:added,overwritten_count=:overwritten,doctor_matched_count=:matched,doctor_unmatched_count=:unmatched,doctor_ambiguous_count=:ambiguous,summary_status='READY',finished_at=CURRENT_TIMESTAMP WHERE id=:id")
                .param("success",rows.size()).param("added",added).param("overwritten",overwritten).param("matched",matched)
                .param("unmatched",unmatched).param("ambiguous",ambiguous).param("id",batchId).update();
        return new Result(batchNo,rows.size(),rows.size(),added,overwritten,0,matched,unmatched,ambiguous);
    }
    static List<DischargeImportRow> readRows(MultipartFile file) {
        return ExcelSheetRows.read(file).stream().filter(row -> ExcelSheetRows.value(row, "住院号") != null).map(row -> {
            ...
            result.gender = ExcelSheetRows.value(row, "性别", "患者性别");
            result.wardName = ExcelSheetRows.value(row, "住院病区", "所属科室");
            result.primaryDiagnosis = ExcelSheetRows.value(row, "主诊断", "主要诊断", "诊断");
            result.secondaryDiagnosis = ExcelSheetRows.value(row, "次要诊断", "副诊断", "其他诊断");
            result.medicalInsuranceType = ExcelSheetRows.value(row, "医保类型", "医疗保险类型");
    ...
    private long insert(DischargeImportRow r,DoctorMatch d){return jdbc.sql("INSERT INTO patient_encounter(inpatient_no,admission_times,patient_name,gender,primary_diagnosis,secondary_diagnosis,medical_insurance_type,department_id,ward_name,fee_type,doctor_name_source,doctor_employee_no,doctor_user_id,doctor_match_status,admitted_at,discharged_at) VALUES (:no,:times,:name,:gender,:diagnosis,:secondaryDiagnosis,:insuranceType,:department,:ward,:fee,:doctor,:employee,:doctorUserId,:doctorStatus,:admitted,:discharged) RETURNING id").param("no",r.inpatientNo.trim()).param("times",r.admissionTimes).param("name",r.patientName.trim()).param("gender",trim(r.gender)).param("diagnosis",trim(r.primaryDiagnosis)).param("secondaryDiagnosis",trim(r.secondaryDiagnosis)).param("insuranceType",trim(r.medicalInsuranceType)).param("department",departmentId(r.wardName)).param("ward",trim(r.wardName)).param("fee",trim(r.feeType)).param("doctor",trim(r.doctorName)).param("employee",trim(r.doctorEmployeeNo)).param("doctorUserId",d.userId()).param("doctorStatus",d.status()).param("admitted",date(r.admittedAt)).param("discharged",date(r.actualDischargeAt)).query(Long.class).single();}
    private void update(long id,DischargeImportRow r,DoctorMatch d){jdbc.sql("UPDATE patient_encounter SET patient_name=:name,gender=COALESCE(:gender,gender),primary_diagnosis=COALESCE(:diagnosis,primary_diagnosis),secondary_diagnosis=COALESCE(:secondaryDiagnosis,secondary_diagnosis),medical_insurance_type=COALESCE(:insuranceType,medical_insurance_type),department_id=COALESCE(:department,department_id),ward_name=COALESCE(:ward,ward_name),fee_type=COALESCE(:fee,fee_type),doctor_name_source=COALESCE(:doctor,doctor_name_source),doctor_employee_no=COALESCE(:employee,doctor_employee_no),doctor_user_id=CASE WHEN CAST(:employee AS VARCHAR) IS NULL THEN doctor_user_id ELSE :doctorUserId END,doctor_match_status=CASE WHEN CAST(:employee AS VARCHAR) IS NULL THEN doctor_match_status ELSE :doctorStatus END,admitted_at=COALESCE(:admitted,admitted_at),discharged_at=COALESCE(:discharged,discharged_at),source_updated_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=:id").param("name",r.patientName.trim()).param("gender",trim(r.gender)).param("diagnosis",trim(r.primaryDiagnosis)).param("secondaryDiagnosis",trim(r.secondaryDiagnosis)).param("insuranceType",trim(r.medicalInsuranceType)).param("department",departmentId(r.wardName)).param("ward",trim(r.wardName)).param("fee",trim(r.feeType)).param("doctor",trim(r.doctorName)).param("employee",trim(r.doctorEmployeeNo)).param("doctorUserId",d.userId()).param("doctorStatus",d.status()).param("admitted",date(r.admittedAt)).param("discharged",date(r.actualDischargeAt)).param("id",id).update();}
    private void upsertDischarge(long id,DischargeImportRow r){jdbc.sql("INSERT INTO discharge_record(encounter_id,planned_discharge_at,actual_discharge_at,planned_discharge_updated_at) VALUES (:id,:planned,:actual,CASE WHEN CAST(:planned AS TIMESTAMPTZ) IS NULL THEN NULL ELSE CURRENT_TIMESTAMP END) ON CONFLICT(encounter_id) DO UPDATE SET planned_discharge_at=COALESCE(EXCLUDED.planned_discharge_at,discharge_record.planned_discharge_at),planned_discharge_updated_at=CASE WHEN EXCLUDED.planned_discharge_at IS NULL THEN discharge_record.planned_discharge_updated_at ELSE CURRENT_TIMESTAMP END,actual_discharge_at=COALESCE(EXCLUDED.actual_discharge_at,discharge_record.actual_discharge_at),updated_at=CURRENT_TIMESTAMP").param("id",id).param("planned",date(r.plannedDischargeAt)).param("actual",date(r.actualDischargeAt)).update();recalculateAbnormalCodes(id);}

    private void recalculateAbnormalCodes(long encounterId){var values=jdbc.sql("SELECT planned_discharge_at,actual_discharge_at,planned_discharge_updated_at FROM discharge_record WHERE encounter_id=:id").param("id",encounterId).query((r,n)->new OffsetDateTime[]{r.getObject(1,OffsetDateTime.class),r.getObject(2,OffsetDateTime.class),r.getObject(3,OffsetDateTime.class)}).single();jdbc.sql("UPDATE discharge_record SET abnormal_codes=:codes WHERE encounter_id=:id").param("codes",String.join(",",DischargeAbnormalCalculator.calculate(values[0],values[1],values[2]))).param("id",encounterId).update();}
