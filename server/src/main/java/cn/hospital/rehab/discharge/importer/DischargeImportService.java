...
public class DischargeImportService {
    ...
    private long insert(DischargeImportRow r,DoctorMatch d){return jdbc.sql("INSERT INTO patient_encounter(inpatient_no,admission_times,patient_name,gender,primary_diagnosis,medical_insurance_type,department_id,ward_name,fee_type,doctor_name_source,doctor_employee_no,doctor_user_id,doctor_match_status,admitted_at,discharged_at) VALUES (:no,:times,:name,:gender,:diagnosis,:insuranceType,:department,:ward,:fee,:doctor,:employee,:doctorUserId,:doctorStatus,:admitted,:discharged) RETURNING id").param("no",r.inpatientNo.trim()).param("times",r.admissionTimes).param("name",r.patientName.trim()).param("gender",trim(r.gender)).param("diagnosis",trim(r.primaryDiagnosis)).param("insuranceType",trim(r.medicalInsuranceType)).param("department",departmentId(r.wardName)).param("ward",trim(r.wardName)).param("fee",trim(r.feeType)).param("doctor",trim(r.doctorName)).param("employee",trim(r.doctorEmployeeNo)).param("doctorUserId",d.userId()).param("doctorStatus",d.status()).param("admitted",date(r.admittedAt)).param("discharged",date(r.actualDischargeAt)).query(Long.class).single();}
    private void update(long id,DischargeImportRow r,DoctorMatch d){jdbc.sql("UPDATE patient_encounter SET patient_name=:name,gender=COALESCE(:gender,gender),primary_diagnosis=COALESCE(:diagnosis,primary_diagnosis),medical_insurance_type=COALESCE(:insuranceType,medical_insurance_type),department_id=COALESCE(:department,department_id),ward_name=COALESCE(:ward,ward_name),fee_type=COALESCE(:fee,fee_type),doctor_name_source=COALESCE(:doctor,doctor_name_source),doctor_employee_no=COALESCE(:employee,doctor_employee_no),doctor_user_id=CASE WHEN CAST(:employee AS VARCHAR) IS NULL THEN doctor_user_id ELSE :doctorUserId END,doctor_match_status=CASE WHEN CAST(:employee AS VARCHAR) IS NULL THEN doctor_match_status ELSE :doctorStatus END,admitted_at=COALESCE(:admitted,admitted_at),discharged_at=COALESCE(:discharged,discharged_at),source_updated_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=:id").param("name",r.patientName.trim()).param("gender",trim(r.gender)).param("diagnosis",trim(r.primaryDiagnosis)).param("insuranceType",trim(r.medicalInsuranceType)).param("department",departmentId(r.wardName)).param("ward",trim(r.wardName)).param("fee",trim(r.feeType)).param("doctor",trim(r.doctorName)).param("employee",trim(r.doctorEmployeeNo)).param("doctorUserId",d.userId()).param("doctorStatus",d.status()).param("admitted",date(r.admittedAt)).param("discharged",date(r.actualDischargeAt)).param("id",id).update();}
